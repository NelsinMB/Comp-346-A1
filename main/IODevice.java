...
public class IODevice {
    ...
    /*
     * Column 0: ID
     * Column 1: Time left (begins at 5). Once 0 is reached, remove from list, add
     * to readyQueue. ***Might need to consider whether it was the last instruction
     */
    ...
    public void decrementWaits() {
        for (Map.Entry<Process, Integer> entry : waitQueue.entrySet()) {
            if (entry.getValue() != 0) {
                ...
                if (entry.getKey().getPCB().getProgramCounter() == entry.getKey().numberOfInstructions) { //Why no -1 here?
